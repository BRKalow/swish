...
class Team < ActiveRecord::Base
  ...
  has_many :members, -> { where(joinings: { status: Joining.statuses[:accepted] }) }, through: :joinings, source: :user
  has_many :pending, -> { where(joinings: { status: Joining.statuses[:pending] }) }, through: :joinings, source: :user
